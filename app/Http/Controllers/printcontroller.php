...
class printcontroller extends Controller
{
    ...
    public function index($id){
    	...
    	$tableOrangTua=DB::table('tb_orang_tua')->where('dosen_id',$id)->
        where(function($query){
            $query->where('jenis','=','Ayah')
            ->orWhere('jenis','=','Ibu');
            })->get();
        $tableOrangTuaMertua=DB::table('tb_orang_tua')->where('dosen_id',$id)->
        where(function($query){
            $query->where('jenis','=','Ayah Mertua')
            ->orWhere('jenis','=','Ibu Mertua');
            })->get();
    	...
    	$tableOrganisasi=DB::table('tb_ket_organisasi')->where('dosen_id',$id)->
        where(function($query){
            $query->where('jenis','=','kuliah');
            })->get();
       
        $tableOrganisasiSlta=DB::table('tb_ket_organisasi')->where('dosen_id',$id)->
        where(function($query){
            $query->where('jenis','=','SD')
            ->orWhere('jenis','=','SMP')
            ->orWhere('jenis','=','SMA');
        })->get();
        $tableOrganisasiPeg=DB::table('tb_ket_organisasi')->where('dosen_id',$id)->
        where(function($query){
    	...
    	return view('print2' )
                ...
                ->with('orangtuamertua',$tableOrangTua);
